...
export async function syncWindsurf(app: App, homeDir: string, outputFolder: string): Promise<number> {
  const outputDir = `${outputFolder}/windsurf-sessions`;
  
  // Find Windsurf storage paths
  const windsurfPaths = [
    path.join(homeDir, 'Library/Application Support/Windsurf'),
    path.join(homeDir, 'Library/Application Support/Codeium'),
    path.join(homeDir, '.config/Windsurf'),
    path.join(homeDir, '.config/Codeium'),
  ];
  
  let syncedCount = 0;
  
  for (const windsurfPath of windsurfPaths) {
    if (!fs.existsSync(windsurfPath)) continue;
    
    const sessionFiles = findSessions(windsurfPath);
    
    for (const sessionFile of sessionFiles) {
      const session = parseSession(sessionFile);
      if (!session || session.messages.length === 0) continue;
      
      const filename = `windsurf-${session.date}-${session.time.replace(':', '')}-${session.sessionId}.md`;
...
function findSessions(windsurfPath: string): string[] {
  const sessions: string[] = [];
  
  const possibleDirs = [
    path.join(windsurfPath, 'User', 'History'),
    path.join(windsurfPath, 'User', 'globalStorage'),
    path.join(windsurfPath, 'chats'),
    path.join(windsurfPath, 'conversations'),
  ];
...
function parseSession(filePath: string): WindsurfSession | null {
  let data: unknown;
  try {
  ...
  if (Array.isArray(data)) {
    ...
    history = (d.messages as unknown[]) || (d.history as unknown[]) || (d.conversation as unknown[]) || [];
  }
  ...
  for (const entry of history) {
    ...
    let content = (e.content as unknown) || (e.text as unknown) || (e.message as unknown) || '';
    ...
    if (Array.isArray(content)) {
      ...
      for (const item of content) {
        if (typeof item === 'string') {
          ...
          if (i.text) parts.push(String(i.text));
        }
      }
      content = parts.join('\n');
    }
...
function generateMarkdown(session: WindsurfSession): string {
...
# 🏄 Windsurf Session — ${session.date} ${session.time.replace(':', '')}
